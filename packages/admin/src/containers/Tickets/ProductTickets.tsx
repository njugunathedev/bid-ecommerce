...
import BorderProgressBar from '../../components/TicketProgressBar/BidProgressBar';
import NoResult from '../../components/NoResult/NoResult';
import { isTemplateExpression } from 'typescript';
import CloseProduct from './CloseTickets';
...
`;
//ToDo Check if all tickets are bought
//ToDo close the biding
//ToDo randomly get the winning Ticket
//ToDo change the specific ticket
//ToDo recreate the product
...
export default function Tickets() {
    ...
    const [closeProductTicket] = useMutation(CLOSE_TICKET);
    ...
    function onAllCheck(event) {
        if (event.target.checked) {
            const idx = data && data.products.items.map(product => product.id);
            setCheckedId(idx);
        } else {
            setCheckedId([]);
        }
        ...
        <Grid fluid={true}>
            <Row>
                <Col md={12}>
                    ...
                    >
                        <Col md={3} xs={12}>
                            <Heading>Product Tickets</Heading>
                        </Col>
                    ...
                    <Wrapper style={{ boxShadow: '0 0 5px rgba(0, 0 , 0, 0.05)' }}>
                        <TableWrapper>
                            <StyledTable $gridTemplateColumns='minmax(70px, 70px) minmax(70px, 70px) minmax(150px, auto) minmax(150px, auto) minmax(200px, max-content) minmax(150px, auto) minmax(150px, auto) minmax(150px, auto) minmax(150px, auto)'>
                                ...
                                <StyledHeadCell>Total tickets</StyledHeadCell>
                                <StyledHeadCell>Amount</StyledHeadCell>
                                <StyledHeadCell>Progress</StyledHeadCell>
                                <StyledHeadCell>Tickets bought</StyledHeadCell>

                                <StyledHeadCell>Close Ticket </StyledHeadCell>
                                        ...
                                        data.products.items
                                            ...
                                            .map((row, index) => (
                                                <React.Fragment key={index}>
                                                    ...
                                                    <StyledCell><BorderProgressBar total={row[5].length} /></StyledCell>
                                                    <StyledCell>{row[5].length}</StyledCell>
                                                    <StyledCell><CloseProduct productId={row[0]} /></StyledCell>
                                                    {console.log(row[0])}
